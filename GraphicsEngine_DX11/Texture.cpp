...
namespace GraphicsEngine
{
    ...
    void Texture::Initialize(const std::wstring& path)
	{
        ...
        HRESULT hr = ::CreateTexture(Graphics_Interface::Get()->GetDevice().Get(), _image.GetImages(), _image.GetImageCount(), _image.GetMetadata(),
        ...
        hr = Graphics_Interface::Get()->GetDevice()->CreateShaderResourceView(_texResource.Get(), &srvDesc, _mapSRV.GetAddressOf());
        ...
        if (textureDesc.MipLevels == 0)
            Graphics_Interface::Get()->GetDeviceContext()->GenerateMips(this->_mapSRV.Get());
	}
